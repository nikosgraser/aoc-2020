...
class ConwaySystem:
    ...
    def __init__(self):
        self.state: Dict[Tuple[int, int, int, int], bool] = defaultdict(bool)

    def __str__(self):
        perimeter = self.perimeter()
        result = ''
        for x3 in range(perimeter[3][0] + 1, perimeter[3][1]):
    ...
    def import_state(self, initial_state: List[str]):
        for x1, line in enumerate(initial_state):
            for x0, char in enumerate(line):
                self.state[x0, x1, 0, 0] = ConwaySystem.conway_state_from_char(char)

    def perimeter(self) -> Tuple[Tuple[int, int], Tuple[int, int], Tuple[int, int], Tuple[int, int]]:
        def dim(d: int):
            return lambda x: x[d]
        ...
        return (
            ...
            (min(active, key=dim(3))[3] - 1, max(active, key=dim(3))[3] + 1),
        )

    def num_active_neighbors_at(self, x0: int, x1: int, x2: int, x3: int) -> int:
        neighborhood = filter(lambda x: x != (x0, x1, x2, x3),
                              product(
                                  range(x0 - 1, x0 + 2),
                                  range(x1 - 1, x1 + 2),
                                  range(x2 - 1, x2 + 2),
        ...
        return sum(1 for n0, n1, n2, n3 in neighborhood if self.state[n0, n1, n2, n3])

    def successor_at(self, x0: int, x1: int, x2: int, x3: int) -> bool:
        if self.state[x0, x1, x2, x3]:
            return self.num_active_neighbors_at(x0, x1, x2, x3) in ConwaySystem.STAY_ACTIVE_NEIGHBORS
        return self.num_active_neighbors_at(x0, x1, x2, x3) in ConwaySystem.BECOME_ACTIVE_NEIGHBORS

    def iterate(self):
        next_state: Dict[Tuple[int, int, int, int], bool] = defaultdict(bool)
        perimeter = self.perimeter()
        for x0, x1, x2, x3 in product(
                range(perimeter[0][0], perimeter[0][1] + 1),
                range(perimeter[1][0], perimeter[1][1] + 1),
                range(perimeter[2][0], perimeter[2][1] + 1),
                range(perimeter[3][0], perimeter[3][1] + 1),
        ):
            next_state[x0, x1, x2, x3] = self.successor_at(x0, x1, x2, x3)
